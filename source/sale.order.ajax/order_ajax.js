	...
	BX.Sale.OrderAjaxComponent = {
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-sm-5 bx-soa-pp-desc-container'}}),
				...
				clear, infoList, extraServices, extraServicesNode, addDiv, labelAddr, inputAddr, inputDistance;
			...
			addDiv = BX.create('DIV');
if(currentDelivery.ID == 11)
{

	var addrProp, distanceProp, autocompleteKladr, noteAddr, resultDistance;
	addrProp = '';
	distanceProp = '';

	this.result.ORDER_PROP.properties.forEach(function(entry) {
		if(entry.ID == 20) addrProp = entry.VALUE[0];
		if(entry.ID == 21) distanceProp = entry.VALUE[0];
	});
	labelAddr = BX.create('span', {
		props: {
			className: 'addrLabel',
		},
		text: 'Введите адрес доставки'
	});
	inputAddr = BX.create('INPUT', {
		props: {
			type: 'text',
			name: 'ORDER_PROP_20',
	...
	inputDistance = BX.create('INPUT',{
		props: {
			type: 'hidden',
			name: 'ORDER_PROP_21',
			className: 'hiddistance',
			value: distanceProp
		}
	});

	autocompleteKladr = BX.create('DIV', {
		props: {
			className: 'autocomplate-kladr'
		}
	});

	addDiv = BX.create('DIV', {
		props: {className: 'addAddres'}
	});
	addDiv.appendChild(labelAddr);

	addDiv.appendChild(inputAddr);
	addDiv.appendChild(autocompleteKladr);
	addDiv.appendChild(inputDistance);
	...
	BX.bind(inputAddr, 'keyup',function(e) {
		if (inputAddr.value.length > 3) {
			BX.ajax({
				url: '/ajax/getDistance.php?addr=' + inputAddr.value,
				dataType: 'html',
				onsuccess: function(data){
					$(autocompleteKladr).show();
					$(autocompleteKladr).html(data);
        },
			});
		}
	});
	...
	// $('<div><label>Введите адрес доставки</label><input type="text" name="ORDER_PROP_12" /></div>').insertBefore($(clear));
}

deliveryInfoContainer.appendChild(
	BX.create('DIV', {
		props: {className: 'bx-soa-pp-company'},
		children: [subTitle, label, title, clear, addDiv, extraServicesNode, infoList]
	})
);
...
getPropertyRowNode: function(property, propsItemsContainer, disabled)
{
	...
	// исключаем вывод св-в
	if (property.getId() != 20 && property.getId() != 21) {
